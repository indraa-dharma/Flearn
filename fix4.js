...
const idStr = `const translations = {
  en: {
    nav: {
      features: "Features",
    ...
    hero: {
      tag: "AI-Powered Academic Intelligence",
      title1: "Stop reacting to deadlines.",
      title2: "Start outsmarting them.",
    ...
    stats: [
      { value: "2.4×", label: "Study efficiency", accent: "text-primary dark:text-blue-400" },
      { value: "89%", label: "Task completion", accent: "text-success dark:text-green-400" },
      { value: "5.2h", label: "Saved per week", accent: "text-amber-600 dark:text-amber-400" },
      { value: "12k+", label: "Students using it", accent: "text-sky dark:text-sky-400" },
    ],
    featuresSection: {
      sub: "Core Pillars",
      title: "Four things that change how you study",
      items: [
        { icon: "FileText", title: "Document Intelligence", description: "Upload any lecture, reading, or syllabus. Get instant AI summaries, flashcards, and concept maps.", color: "bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-950/40 dark:text-blue-400 dark:border-blue-900/60", accent: "border-l-blue-500 dark:border-l-blue-400" },
        { icon: "Calendar", title: "Calendar Sync", description: "One source of truth for exams, deadlines, and AI-scheduled study blocks — all in one view.", color: "bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-900/60", accent: "border-l-amber-500 dark:border-l-amber-400" },
        { icon: "BarChart3", title: "Priority Engine", description: "A live urgency score on every task. Know exactly what to tackle first and why — not just when.", color: "bg-red-50 text-red-600 border-red-100 dark:bg-red-950/40 dark:text-red-400 dark:border-red-900/60", accent: "border-l-red-500 dark:border-l-red-400" },
        { icon: "BookOpen", title: "Study Plans", description: "Your day, built by AI from your calendar, documents, and learning history. Adapts daily.", color: "bg-green-50 text-green-600 border-green-100 dark:bg-green-950/40 dark:text-green-400 dark:border-green-900/60", accent: "border-l-green-500 dark:border-l-green-400" },
      ],
    },
    howItWorksSection: {
      sub: "Process",
      ...
      items: [
        { num: "01", title: "Upload & Connect", desc: "Drop in your syllabi, lecture slides, and notes. Connect Google Calendar in one click.", color: "from-primary to-sky" },
        { num: "02", title: "AI Analyzes", desc: "Your documents are processed, key concepts extracted, and cross-referenced with your deadlines.", color: "from-sky to-green-400" },
        { num: "03", title: "Study Smarter", desc: "Get a scored task list, a concrete daily plan, and an AI you can ask anything — all grounded in your notes.", color: "from-green-400 to-amber-400" },
      ],
    },
    benefitsSection: {
      sub: "Why it works",
      title: "Built for students who actually want to perform",
      desc: "Every feature was designed around one question: what does a student need to stop wasting time and start making progress on what counts?",
      list: [
        "AI summaries and key concept extraction from any document",
        "Smart priority scoring across urgency, difficulty, and grade impact",
        "Personalized daily study timelines that adapt to your schedule",
        "Google Calendar integration for conflict detection and study gaps",
        "Focus session tracking with real productivity analytics",
        "Source-grounded AI chat — answers tied to your actual notes",
      ],
      cta: "Start your free trial",
      stats: [
        { icon: "BarChart3", value: "2.4×", label: "Average study efficiency improvement", color: "text-primary dark:text-blue-400", bg: "bg-card", border: "border-border" },
        { icon: "CheckCircle", value: "89%", label: "Task completion rate with AI planning", color: "text-success dark:text-green-400", bg: "bg-card", border: "border-border" },
        { icon: "Zap", value: "5.2h", label: "Average time saved per week", color: "text-amber-600 dark:text-amber-400", bg: "bg-card", border: "border-border" },
        { icon: "Brain", value: "12k+", label: "Students using FLearn monthly", color: "text-sky dark:text-sky-400", bg: "bg-card", border: "border-border" },
      ],
    },
    ctaSection: { sub: "Get started today", title: "Your AI academic advisor is waiting.", desc: "Join thousands of students who've replaced panic-studying with intelligent planning.", cta1: "Start Free" },
    footer: { copy: "© 2025 FLearn AI", links: ["Support", "Privacy Policy", "Terms of Service"] },
  },
  id: {
...
// Also replace string icons with actual components.
const finalIdStr = idStr.replace(/"FileText"/g, 'FileText').replace(/"Calendar"/g, 'Calendar').replace(/"BarChart3"/g, 'BarChart3').replace(/"BookOpen"/g, 'BookOpen').replace(/"CheckCircle"/g, 'CheckCircle').replace(/"Zap"/g, 'Zap').replace(/"Brain"/g, 'Brain');

const start = content.indexOf('const translations = {');
const end = content.indexOf('const languages = [');

if (start > -1 && end > -1) {
    content = content.substring(0, start) + finalIdStr + '\n\n' + content.substring(end);
    
    // Also fix the languages array
